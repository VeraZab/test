            ...
            <ContentSection
              ...
              style={{
                background: section.content[0].showBackground
                  ? `url(${section.content[0].image})`
                  : '',
              }}
            >
              <ContentPane half graphic>
                {section.content[0].showBackground
                  ? null
    ...
    const learnMoreItems = data.learnMore ? (
      <ContentSection>
        <ContentPane full center-vertically text center>
          <div className="pre-title">Resources</div>
          <Title>{data.learnMore.name}</Title>
          <Body>
        ...
        {gridItems ? (
          <ContentSection>
            <ContentPane full center>
              <Body>
                <Grid columns={gridItems.length > 5 ? 4 : gridItems.length}>
                  {gridItems}
                </Grid>
              </Body>
            </ContentPane>
  ...
  getQuoteWithIndexFromData(index, data) {
    ...
    return data.quotes[index] ? (
      <ContentSection className={contentSectionClass + ' quote-section'}>
        <ContentPane full center-vertically text center>
          <Body>
            <div className="quote">"{data.quotes[index].desc}"</div>
